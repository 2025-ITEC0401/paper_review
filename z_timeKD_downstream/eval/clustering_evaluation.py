...
def evaluate(pred_res_csv, gt_label_csv, label_column_name, seq_len, n_vars):
    try:
...
for ds in DATASET:
    for output in OUTPUT_LEN:
        for method in ['kmeans', 'spectral']:
            pred_csv = f"{RES_DIR}/csv/{ds}_o{output}_{method}_res.csv"
            ...
            evaluate(pred_res_csv=pred_csv, gt_label_csv=gt_csv, label_column_name='label', seq_len=seq_len, n_vars=var)