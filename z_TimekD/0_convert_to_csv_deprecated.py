...
def convert_to_CSV(filename):
    try:
            ...
            with open(filename, 'r') as f:
                for line in f:
                    ...
                    if (line.strip().lower().startswith('@attribute')):
                        last_attr_line = line.strip()
            print(last_attr_line.split()[1])
            return
        else:
        ...
        df_flat.insert(0, last_attr_line.split()[1], label)
...
for ds in ['BasicMotions', 'DuckDuckGeese', 'Epilepsy']:
    ...
    for filepath in os.listdir(path):
        if (filepath.split(".")[-1] == ('arff' or 'ts')):
            convert_to_CSV(filename=f"{path}/{filepath}")
