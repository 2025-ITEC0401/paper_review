...
def generate_markdown_report(all_results, output_dir):
    ...
    # Clustering 결과 표
    lines.append("\n## 2. Clustering 결과\n")
    lines.append("### KMeans vs Spectral Clustering 비교\n")
    lines.append("| Dataset | Method | NMI | RI | ARI | Silhouette | Clusters |")
    lines.append("|---------|--------|-----|-----|-----|------------|----------|")

    for dataset, results in all_results.items():
        for method_key in ['clustering_kmeans', 'clustering_spectral']:
            if method_key in results:
                cl = results[method_key]
                method_name = cl.get('method', method_key.split('_')[-1]).upper()
                lines.append(
                    f"| {dataset} | {method_name} | {cl['nmi']:.4f} | {cl['ri']:.4f} | {cl['ari']:.4f} | "
    ...
    kmeans_nmis = [r['clustering_kmeans']['nmi'] for r in all_results.values() if 'clustering_kmeans' in r]
    if kmeans_nmis:
        lines.append(f"- **Clustering (KMeans) 평균 NMI**: {np.mean(kmeans_nmis):.4f}\n")

    spectral_nmis = [r['clustering_spectral']['nmi'] for r in all_results.values() if 'clustering_spectral' in r]
    if spectral_nmis:
        lines.append(f"- **Clustering (Spectral) 평균 NMI**: {np.mean(spectral_nmis):.4f}\n")
...
def main(args):
    ...
    for dataset in DATASETS:
        ...
        if 'clustering' in tasks and '_cluster_labels_kmeans' in results:
            print(f"  [Visualization] Creating clustering plot (KMeans)...")
            visualize_clustering(
                representations,
                results['_cluster_labels_kmeans'],
                labels,
                dataset + '_kmeans',
                figures_dir
            )
        if 'clustering' in tasks and '_cluster_labels_spectral' in results:
            print(f"  [Visualization] Creating clustering plot (Spectral)...")
            visualize_clustering(
                representations,
                results['_cluster_labels_spectral'],
                labels,
                dataset + '_spectral',
                figures_dir
            )
